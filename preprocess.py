...
def preprocess_data(dir_path: Path):
    ...
    for full_path in image_paths:
        ...
        # cyy：ltype=None 自动选择标签类型，小图用16足够
        num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(
            thresh.copy(), connectivity=8, ltype=None
        )
        # 加入连通块外接矩阵几何中心
        # cyy：换成astype，int16足够表达
        for i in range(1, num_labels):  # 从1开始，跳过背景区域
            x, y, width, height, area = stats[i]
            centre_x = (x + width // 2).astype(np.int16)
            centre_y = (y + height // 2).astype(np.int16)
            pp.append([centre_x, centre_y])
        for pi in centroids[1:]:
            px, py = (pi[0]).astype(np.int16), (pi[1]).astype(np.int16)
            pp.append([px, py])
    pp = np.array(pp)
    # 去重

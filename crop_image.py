    ...
    for i in range(tile_number):
        for j in range(tile_number):
            for path in images:
                ...
                output_path = os.path.join(src_folder, f'{tile_number}', f'{new_folder_name}_{tile_number * tile_number}', os.path.basename(path))
                os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_tile", type=int, default=4)
    ...
    print(f"Image cropped and saved to {args.src} / {args.n_tile}")
...
from systems import GSSystem3DPerCam
from datasets import RNADataset3DPerCam
...
def main():
    ...
    # model & dataloader
    train_dataset = RNADataset3DPerCam(hparams=config, mode='train')
    train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=8)

    val_dataset = RNADataset3DPerCam(hparams=config, mode='val')
    ...
    gs_system = GSSystem3DPerCam(hparams=config, dapi_images=train_dataset.dapi_images)
    ...
    gs_system = GSSystem3DPerCam.load_from_checkpoint(checkpoint_callback.best_model_path)
